...
- **AI 에이전트 지원** — Claude Code, Cursor, GitHub Copilot, Windsurf 등 주요 AI 코딩 도구에 대응하는 인스트럭션 파일 내장.
...
이 템플릿은 AI 코딩 도구가 프로젝트 구조를 이해하고 커스터마이징을 도와줄 수 있도록 다양한 에이전트 인스트럭션 파일을 포함합니다.

| 파일 | 대응 도구 |
|------|----------|
| `AGENTS.md` | OpenAI Codex, Gemini, Devin, Cursor, Zed, Aider 등 (업계 표준) |
| `CLAUDE.md` | Claude Code / OpenCode |
| `.github/copilot-instructions.md` | GitHub Copilot |
| `.cursor/rules/project.mdc` | Cursor (glob 기반 규칙) |
| `.windsurfrules` | Windsurf |

AI 에이전트에게 "**내 포트폴리오를 커스터마이징해줘**"라고 요청하면, 에이전트가 자동으로 프로젝트 구조와 컨벤션을 파악하고 올바른 파일을 수정합니다.
