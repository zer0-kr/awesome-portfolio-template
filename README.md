...
- **경력 상세 페이지** — 타임라인만으로는 부족합니다. `/career` 페이지에서 프로젝트별 배경, 역할, 성과, 교훈을 깊이 있게 전달할 수 있습니다.
- **다국어 기본 내장** — `next-intl` 기반 URL 라우팅(`/ko`, `/en`)까지 완전히 분리되어 있어, 언어 추가도 간단합니다.
- **설정 파일 하나로 끝** — `src/config/site.ts`와 데이터 파일 7개만 수정하면 나만의 포트폴리오가 완성됩니다. 컴포넌트를 직접 수정할 필요가 없습니다.
- **완성도 높은 디자인** — Framer Motion 애니메이션, 그라데이션 오브, 스크롤 인디케이터까지. 직접 만들면 몇 주 걸릴 결과물을 바로 쓸 수 있습니다.
